...
def _apply_custom_montage_keep_all(raw: mne.io.BaseRaw, montage_path: Optional[Path] = None, montage_kwargs: Optional[dict] = None):
...
def _plot_topomap_with_highlights(ax, data, info, raw, title, highlights, cmap):
    ...
    im, _ = mne.viz.plot_topomap(
        ...
        res=256, sphere=(0.0, 0.0, 0.0, 0.095),
        extrapolate="local"
    )
...
def _draw_bandpower_with_highlight(raw_before: mne.io.BaseRaw,
    ...
    psd_b = raw_before.compute_psd(fmin=fmin, fmax=fmax, method="welch", n_fft=2048, verbose=False)
    psd_a = raw_after.compute_psd(fmin=fmin, fmax=fmax, method="welch", n_fft=2048, verbose=False)
    ...
    _plot_topomap_with_highlights(axes[0], pb, info_b, raw_before, "a) Raw Bandpower\nDropped Sensors (Yellow)",
                                 {"removed": (removed_mapped, COLOR_REMOVED)}, cmap)
    _plot_topomap_with_highlights(axes[1], pa, info_a, raw_after, "b) Cleaned Bandpower\nFixed (Green) | Dropped (Yellow)",
                                 {"fixed": (fixed_mapped, COLOR_FIXED), "removed": (removed_mapped, COLOR_REMOVED)}, cmap)
...
def _draw_psd_split(raw_before: mne.io.BaseRaw, raw_after: mne.io.BaseRaw, axes):
    _ensure_montage(raw_before)
    _ensure_montage(raw_after)
    raw_before.plot_psd(fmin=1, fmax=40, ax=axes[0], average=False, picks="eeg", show=False)
    axes[0].set_title("Raw PSD", fontsize=11)
    try:
        ra_refed = mne.set_eeg_reference(raw_after.copy())[0]
    except Exception:
        ra_refed = raw_after
    ra_refed.plot_psd(fmin=1, fmax=40, ax=axes[1], average=False, picks="eeg", show=False)
...
def create_bandpower_psd_flagged(
    ...
    try:
        raw_before = _drop_duplicate_coords(mne.io.read_raw_brainvision(str(vhdr_path), preload=True, verbose=False))
        raw_after = _drop_duplicate_coords(mne.io.read_raw_fif(str(fif_path), preload=True, verbose=False))
        ...
        ch_col = next((c for c in df_flags.columns if c.lower() in ["ch_name", "ch_names", "channel", "name"]), None)
        flag_col = next((c for c in df_flags.columns if c.lower() in ["flag", "status", "reason", "labels", "note", "type"]), None)
        if not ch_col or not flag_col:
            raise ValueError("Could not find channel/flag columns in flagged file")

        df_flags["ch_norm"] = df_flags[ch_col].astype(str).str.replace("EEG ", "", regex=False).str.lower().str.strip()
        df_flags["flag_norm"] = df_flags[flag_col].astype(str).str.lower()

        removed = df_flags.loc[df_flags["flag_norm"].str.contains(
            "uncorrelated|flat|bad|remove|drop|noisy|rank", na=False), "ch_norm"].tolist()

        rb_names = [ch.lower().replace("eeg ", "").strip() for ch in raw_before.ch_names]
        removed_mapped = [raw_before.ch_names[i] for i, ch in enumerate(rb_names) if ch in removed]
        ...
        if out_path.exists() and not overwrite:
            return out_path, {"status": "Exists", "message": "File already exists", "dropped": removed_mapped, "fixed": fixed_mapped, "out_path": str(out_path)}
        ...
        _draw_bandpower_with_highlight(raw_before, raw_after, [ax_raw, ax_clean], removed_mapped, fixed_mapped, fmin=fmin, fmax=fmax)
        ...
        return out_path, {"status": "Success", "message": "Saved", "dropped": removed_mapped, "fixed": fixed_mapped, "out_path": str(out_path)}
    except Exception as e:
        return None, {"status": "Error", "message": f"{type(e).__name__}: {e}"}


def process_subject_from_paths(vhdr_path: Path, root_clean_dir: Path, root_pylossless_dir: Path, out_dir: Path, montage_path: Optional[Path] = None, overwrite: bool = False):
    """Given a BrainVision vhdr and search dirs, locate matching cleaned .fif and flagged tsv and run create_bandpower_psd_flagged().

    This mirrors the behavior in your script but exposes behavior as one function.
    """
    ...
    fif = next(Path(root_clean_dir).rglob(f"{subj_name}*_clean*.fif"), None)
    flagged_file = next(Path(root_pylossless_dir).rglob(f"{subj_name}*_FlaggedChs.tsv"), None)
    if not fif or not flagged_file:
        msg = "Missing FIF or TSV file"
        return None, {"status": "FileMissing", "message": msg, "vhdr": str(vhdr_path), "fif": str(fif) if fif else None, "flagged": str(flagged_file) if flagged_file else None}

    return create_bandpower_psd_flagged(vhdr_path, fif, flagged_file, out_dir, montage_path=montage_path, overwrite=overwrite)
