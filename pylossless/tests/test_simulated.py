...
cov_noisy = make_ad_hoc_cov(
    raw_sim.copy().pick(make_these_noisy).info, std=dict(eeg=0.000002)
)
...
cov_less_noisy = make_ad_hoc_cov(
    (raw_selection1.copy().pick(make_these_noisy).info), std=dict(eeg=0.0000008)
)
add_noise(
    raw_selection1, cov_less_noisy, iir_filter=[0.2, -0.2, 0.04], random_state=rng
)
...
min_val = data[23, :].min()
max_val = data[23, :].min() + 0.0000065
low_correlated_ch = np.random.uniform(low=min_val, high=max_val, size=len(data[23, :]))
...
config["ch_ch_sd"]["outliers_kwargs"]["lower"] = 0.15
config["ch_ch_sd"]["outliers_kwargs"]["upper"] = 0.85

config["epoch_ch_sd"]["outliers_kwargs"]["k"] = 3
config["epoch_ch_sd"]["outliers_kwargs"]["lower"] = 0.15
config["epoch_ch_sd"]["outliers_kwargs"]["upper"] = 0.85

config["ch_low_r"]["outliers_kwargs"]["k"] = 2
config["ch_low_r"]["outliers_kwargs"]["lower"] = 0.23
config["ch_low_r"]["outliers_kwargs"]["upper"] = 0.85
config["ch_low_r"]["flag_crit"] = 0.25

config["epoch_low_r"]["outliers_kwargs"]["k"] = 3
config["epoch_low_r"]["outliers_kwargs"]["lower"] = 0.15
config["epoch_low_r"]["outliers_kwargs"]["upper"] = 0.85
