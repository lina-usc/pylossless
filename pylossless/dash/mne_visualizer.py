...
class MNEVisualizer:
    ...
    def __init__(
        ...
        self.dash_ids = {
            id_: (id_ + f"_{dash_id_suffix}") if dash_id_suffix else id_
        ...
        self.dcc_graph_kwargs = dict(
            id=self.dash_ids["graph"],
        ...
        self.graph_div = html.Div(
            [self.graph], style=STYLE["timeseries-div"], className=CSS["timeseries-div"]
        )
    ...
    def load_recording(self, raw):
        ...
        self.time_slider.marks = {int(key): str(int(key)) for key in marks_keys}
    ...
    def initialize_layout(self):
        ...
        DEFAULT_LAYOUT["yaxis"].update(
            {
                "tickvals": tickvals_handler,
        ...
        self.traces = [
            go.Scatter(name=ii, **trace_kwargs) for ii in range(self.n_sel_ch)
        ]
    ...
    def update_layout(self, ch_slider_val=None, time_slider_val=None):
        ...
        ch_zip = zip(range(1, self.n_sel_ch + 1), ch_names, data, self.traces, ch_types)
    ...
    def set_callback(self):
        ...
        args = [
            Output(self.dash_ids["graph"], "figure"),
        ...
        @self.app.callback(
            *args, suppress_callback_exceptions=False, prevent_initial_call=False
        )
        def callback(ch, time, click_data, relayout_data, *args):
            ...
            update_layout_ids = [
                self.dash_ids["ch-slider"],
                self.dash_ids["time-slider"],
            ]
            if self.refresh_inputs:
                update_layout_ids.extend(
                    [inp.component_id for inp in self.refresh_inputs]
                )
                ...
                if object_ == self.dash_ids["graph"]:
                    if dash_event == "clickData":
                        ...
                        if "selections" in relayout_data:
                            ...
                            annot = EEGAnnotation(
                                onset, offset - onset, description, self.layout
                            )
                            ...
                            if len(updated_shapes) < len(self.layout.shapes):
                                ...
                                previous_names = [
                                    shape["name"] for shape in self.layout.shapes
                                ]
                                new_names = [shape["name"] for shape in updated_shapes]
                                deleted = set(previous_names) - set(new_names)
                                self.mne_annots.data.remove(deleted.pop())

                        elif any([key.endswith("x0") for key in relayout_data.keys()]):
                            # shape was modified
    ...
    def _init_sliders(self):
        ...
        self.channel_slider = dcc.Slider(
            id=self.dash_ids["ch-slider"],
            min=self.n_sel_ch - 1,
        ...
        self.channel_slider_div = html.Div(
            self.channel_slider, className=CSS["ch-slider-div"], style={}
        )
        ...
        self.time_slider = dcc.Slider(
            id=self.dash_ids["time-slider"],
            min=self.times[0],
        ...
        self.time_slider_div = html.Div(
            self.time_slider, className=CSS["time-slider-div"], style={}
        )
    ...
    def _set_div(self):
        ...
        self.container_plot = html.Div(
            id=self.dash_ids["container-plot"],
...
class ICVisualizer(MNEVisualizer):
    ...
    def update_layout(self, ch_slider_val=None, time_slider_val=None):
        ...
        if not self.inst:
            return
        super(ICVisualizer, self).update_layout(ch_slider_val, time_slider_val)
