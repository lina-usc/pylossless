...
# ADD INITIAL/REASONABLE AMOUNT OF NOISE FOR REST OF TIMES
cov = make_ad_hoc_cov(raw_sim.info, std=dict(eeg=0.0000003))
add_noise(raw_sim, cov, iir_filter=[0.2, -0.2, 0.04], random_state=rng)

# MAKE SOME VERY NOISY CHANNELS
make_these_noisy = ['EEG 001', 'EEG 005']
...
make_these_noisy = ['EEG 009', 'EEG 011']

# Make them noisy for about 25% of time
raw_selection1 = raw_sim.copy().crop(tmin=0, tmax=7, include_tmax=False)
raw_selection2 = raw_sim.copy().crop(tmin=7, tmax=19.994505956825666)
...
data[15, :] = data[14, :]  # duplicate the signal

# Try to make chs 27 and 41 slightly more random
# 27 will still be picked up by flag_ch_bridge
# which seems to be very sensitive..
ch_27 = data[26, :5000]  # just add randomness to half the signal..
ch_41 = data[40, 5000:]
data[26, :5000] = ch_27 + np.random.normal(0, .000002, len(ch_27))
data[40, 5000:] = ch_41 + np.random.normal(0, .0000015, len(ch_41))
...
def test_simulated_raw(pipeline):
    ...
    assert np.array_equal(pipeline.flagged_chs['bridge'],
                          ['EEG 015', 'EEG 016', 'EEG 027'])
    assert np.array_equal(pipeline.flagged_chs['manual'],
                          ['EEG 001', 'EEG 005', 'EEG 009',
                           'EEG 011', 'EEG 015', 'EEG 016', 'EEG 027'])
